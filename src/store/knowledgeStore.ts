...
interface KnowledgeState {
  ...
  fetchMetaphors: (title: string, artist: string, lyrics?: string, force?: boolean) => Promise<void>;
  ...
  persist(
    (set, get) => ({
      ...
      fetchBackstory: async (title, artist, force = false) => {
        ...
        const config = aiStore.configs?.find(c => c.id === aiStore.activeConfigId) || (aiStore.config as any);
        ...
        try {
          ...
          const response = await fetch(url, {
            ...
            body: JSON.stringify({
              ...
              messages: [{ role: "system", content: systemPrompt }, { role: "user", content: `歌曲：${title} / ${artist}` }],
              temperature: 0.8,
              max_tokens: 150,
            }),
          });
          ...
          set((state) => ({ backstories: { ...state.backstories, [key]: { content, timestamp: Date.now() } }, isLoading: false }));
        ...
        try {
          const response = await fetch(url, {
            ...
            body: JSON.stringify({
              model: config.model || "gpt-4o-mini",
              messages: [
                { role: "system", content: "你是一位诗学专家。解析意象。JSON: [{\"term\": \"...\", \"meaning\": \"...\"}]。严禁废话。" },
          ...
          set((state) => ({ metaphors: { ...state.metaphors, [key]: parsedMetaphors }, lastRawResponse: rawContent, isLoading: false }));
        } catch (error: any) { set({ isLoading: false, lastRawResponse: error.message }); }
      },
