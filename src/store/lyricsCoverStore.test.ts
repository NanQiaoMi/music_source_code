...
describe("lyricsCoverStore", () => {
  ...
  it("importCoverImage 创建封面", () => {
    const cover = useLyricsCoverStore
      .getState()
      .importCoverImage("s1", "data:image/png;base64,test", "png");
  ...
  it("cropCover 和 resizeCover 方法存在（Canvas 需要在真实浏览器中测试）", () => {
    ...
    useLyricsCoverStore.getState().cropCover("s1", { x: 0, y: 0, width: 100, height: 100 });
