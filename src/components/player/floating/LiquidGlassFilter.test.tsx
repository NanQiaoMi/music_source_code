...
describe("LiquidGlassFilter & Mineradio Liquid Glass Engine", () => {
  ...
  it("exports valid 8-layer physical mimetic shadow constants and CSS class names", () => {
    ...
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[0]).toBe("inset 0 1.5px 2px 0 rgba(255, 255, 255, 0.45)");
    // Layer 2: Internal caustic glow
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[1]).toBe("inset 0 0 24px 2px rgba(255, 255, 255, 0.08)");
    // Layer 3: Bottom ambient bounce
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[2]).toBe("inset 0 -1.5px 2px 0 rgba(255, 255, 255, 0.12)");
    // Layer 4: Deep inner occlusion
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[3]).toBe("inset 0 12px 28px -10px rgba(0, 0, 0, 0.5)");
    // Layer 5: Contact base shadow
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[4]).toBe("0 2px 4px 0 rgba(0, 0, 0, 0.35)");
    // Layer 6: Near-field atmospheric dispersion
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[5]).toBe("0 10px 20px -2px rgba(0, 0, 0, 0.45)");
    // Layer 7: Mid-field volumetric drop
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[6]).toBe("0 24px 48px -6px rgba(0, 0, 0, 0.65)");
    // Layer 8: Far-field ambient dark halo
    expect(MINERADIO_LIQUID_GLASS_SHADOW_LAYERS[7]).toBe("0 40px 80px -12px rgba(0, 0, 0, 0.75)");
  ...
  it("renders SVG displacement filter with 3-channel chromatic dispersion (Red: 180, Green: 170, Blue: 160)", () => {
    ...
    expect(markup).toContain('result="redOnly"');
    expect(markup).toContain('result="greenOnly"');
    expect(markup).toContain('result="blueOnly"');
    expect(markup).toContain('result="chromaticDispersion"');

    // Specular lighting & Glint
    expect(markup).toContain("feSpecularLighting");
    expect(markup).toContain('result="specularGlint"');
  });
