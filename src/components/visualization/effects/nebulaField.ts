...
export const drawNebulaField = ({
  ...
  const globalScale =
    (1 + Math.sin(t * 0.5) * 0.03 + screenPulse + heartbeat) * (effectParams.depth || 1.0);
  ...
  const draw3DPlanes = () => {
    ...
    const drawPlane = (isTop: boolean) => {
      ...
      for (let i = 0; i <= vLines; i++) {
        ...
        ctx.lineTo(xEdge, isTop ? 0 : height);
        ctx.stroke();
      }
      const hLines = 12;
  ...
  if (effectParams.hudDetail > 0.1) {
    ...
    const drawCoreLayer = (
      ...
      rotation: number,
      segments: number,
      style: "solid" | "dashed" | "hex" | "arc" | "dots",
      color: string
    ) => {
      ctx.save();
      ctx.rotate(rotation);
    ...
    drawCoreLayer(
      ...
      3,
      "hex",
      `hsla(${accentHue}, 100%, 90%, ${0.3 + bass * 0.4})`
    );
    drawCoreLayer(
      100 + mid * 20,
      t * 0.5,
      6,
      "hex",
      `hsla(${mainHue}, 100%, 90%, ${0.25 + bass * 0.3})`
    );
    ...
    drawCoreLayer(
      ...
      12,
      "dashed",
      `hsla(${accentHue}, 100%, 80%, ${0.35 + mid * 0.2})`
    );
    ...
    for (let i = 0; i < gyroCount; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        const nx = Math.cos(nAngle) * gRadius,
          ny = Math.sin(nAngle) * gRadius * (0.2 + i * 0.05);
        ctx.fillStyle = j === 0 ? "white" : `hsla(${accentHue}, 100%, 90%, 0.8)`;
        ctx.beginPath();
        ctx.arc(nx, ny, 1.5 + j, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
    }
    ...
    const drawNeuralParticles = () => {
      ...
      for (let i = 0; i < pCount; i++) {
        ...
        const px = Math.cos(pAngle) * pRadius,
          py = Math.sin(pAngle) * pRadius;
        ctx.fillStyle = `hsla(${mainHue}, 100%, 95%, ${0.6 * treble})`;
        ctx.beginPath();
        ctx.arc(px, py, 2 + treble * 4, 0, Math.PI * 2);
        ctx.fill();
      }
      ctx.restore();
    };
  ...
  stars.slice(0, activeStarCount).forEach((s) => {
    ...
    const k = 1000 / s.z;
    const px = s.x * k + cx + cameraDriftX * (1 - s.z / 2500),
      py = s.y * k + cy + cameraDriftY * (1 - s.z / 2500);
