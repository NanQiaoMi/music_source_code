...
export const drawSpectrumRing = ({
  ...
  refs.smoothBass.current =
    (refs.smoothBass.current || 0) * 0.85 + ((getVal(0) + getVal(2)) / 2) * 0.15;
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const smokeG = ctx.createRadialGradient(
      cx + driftX,
      cy + driftY,
      0,
      cx + driftX,
  ...
  for (let i = 0; i < rayCount; i++) {
    ...
    const rAng =
      t * speed * direction + i * ((Math.PI * 2) / rayCount) + Math.sin(t * 0.5 + i) * 0.2;
  ...
  for (let i = 0; i < 15; i++) {
    const rx = -sw + i * 300;
    ctx.beginPath();
    ctx.arc(rx, -sh * 0.38 + 35, 5, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(rx, sh * 0.38 + 35, 5, 0, Math.PI * 2);
    ctx.fill();
  ...
  ctx.beginPath();
  ctx.ellipse(0, 0, orbRadius * 1.2, orbRadius * 0.8, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
  ...
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.arc(sx, sy, 20 + signalT * 80, 0, Math.PI * 2);
  ctx.stroke();
  ctx.restore();
  ...
  ctx.beginPath();
  ctx.arc(0, 0, baseR - 20, 0, Math.PI * 2);
  ctx.stroke();
  ctx.setLineDash([]);
  ctx.restore();
  ...
  for (let i = 1; i <= 4; i++) {
    ctx.beginPath();
    ctx.ellipse(0, 0, emSize * 0.45 * (i / 5), emSize * 0.45, 0, 0, Math.PI * 2);
    ctx.stroke();
    ctx.beginPath();
    ctx.ellipse(0, 0, emSize * 0.45, emSize * 0.45 * (i / 5), 0, 0, Math.PI * 2);
    ctx.stroke();
  }
  ...
  ctx.lineWidth = 6;
  ctx.beginPath();
  ctx.arc(0, 0, wreathR, Math.PI * 0.65, Math.PI * 1.35, false);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(0, 0, wreathR, Math.PI * 1.65, Math.PI * 2.35, false);
  ctx.stroke();
  ...
  for (let i = 0; i < 16; i++) {
    ...
    drawKernel(
      Math.cos(angL) * (wreathR - 14),
      Math.sin(angL) * (wreathR - 14),
      angL - Math.PI / 4
    );
    ...
    drawKernel(
      Math.cos(angR) * (wreathR - 14),
      Math.sin(angR) * (wreathR - 14),
      angR + Math.PI / 4
    );
  }
  ctx.restore();
  ...
  for (let i = 0; i < 7; i++) {
    ...
    ctx.beginPath();
    ctx.arc(0, 0, wreathR - 5, angL, angL + 0.045);
    ctx.stroke();
    const angR = Math.PI * 1.72 + i * 0.1;
    ctx.beginPath();
    ctx.arc(0, 0, wreathR - 5, angR, angR + 0.045);
    ctx.stroke();
  }
  ctx.beginPath();
  ctx.arc(0, wreathR, 45, 0, Math.PI * 2);
  ctx.fillStyle = cRed;
  ctx.fill();
  ctx.restore();
  ...
  const emblemPath = new Path2D(
    "M172 159 c6 7-8 21-15 15 l-83-86-14 14-20-20 32-31 33 6-18 18 m-22 62 -3 3 3 3 c4 4-2 10-6 6 l-3-3 c-5 10-9 14-15 20 -8 8-21-5-13-13 6-6 10-10 20-15 l-3-3 c-4-4 2-10 6-6 l3 3 8-8 h8 c8 12 30 28 57 7 23-18 24-69-26-98 50 4 82 77 41 113 -26 23-64 15-77-9"
  );
  ...
  ctx.moveTo(0, sh);
  ctx.lineTo(cSize, sh);
  ctx.lineTo(0, sh - cSize);
  ctx.fill();
  // Bottom Right Triangle
  ctx.beginPath();
  ctx.moveTo(sw, sh);
  ctx.lineTo(sw - cSize, sh);
  ctx.lineTo(sw, sh - cSize);
  ctx.fill();
  ctx.restore();
  ...
  const drawGauge = (x: number, y: number, label: number, val: number, text: string) => {
    ...
    // Outer arc
    ctx.beginPath();
    ctx.arc(0, 0, 35, Math.PI * 0.8, Math.PI * 2.2);
    ctx.stroke();
    // Ticks
    for (let i = 0; i <= 5; i++) {
      const ta = Math.PI * 0.8 + i * ((Math.PI * 1.4) / 5);
      ctx.beginPath();
      ctx.moveTo(Math.cos(ta) * 30, Math.sin(ta) * 30);
      ctx.lineTo(Math.cos(ta) * 35, Math.sin(ta) * 35);
      ctx.stroke();
    }
    // Needle
    ...
    ctx.beginPath();
    ctx.moveTo(0, 0);
    ctx.lineTo(Math.cos(na) * 32, Math.sin(na) * 32);
    ctx.stroke();
    // Center point
    ctx.fillStyle = cRed;
    ctx.beginPath();
  ...
  corners.forEach(([sx, sy]) => {
    ...
    ctx.fillText(
      `LOC: ${Math.floor(cx + sx * dist)},${Math.floor(cy + sy * dist)}`,
      sx * 10,
      sy * 10
    );
    ctx.restore();
  ...
  for (let i = 0; i < sh; i += 80) {
    ctx.beginPath();
    ctx.arc(icPos - 25, i + 40, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(sw - icPos + 25, i + 40, 6, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
