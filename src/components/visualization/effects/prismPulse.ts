...
export function drawPrismPulse({
  ctx,
  width,
  height,
  data,
  ...
  for (let i = 0; i < labelCount; i++) {
    ...
    const lx = Math.cos(lAngle) * lRadius,
  ...
  if (flareInt > 0.1) {
    ...
    fGrd.addColorStop(0.5, `hsla(${theme.primary}, 100%, 95%, ${flareInt})`);
  ...
  vig.addColorStop(1, `rgba(0, 0, 0, ${0.9 + bass * 0.05})`);
  ctx.fillStyle = vig;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.fillText(
    `DATA_FLUX: ${(energy * 1024).toFixed(0)}KB/S`,
    width - margin,
    height - margin + 12
  );
