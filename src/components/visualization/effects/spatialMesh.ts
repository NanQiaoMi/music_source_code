...
export const drawSpatialMesh = ({ ctx, width, height, data, params, time, refs, theme }: EffectContext) => {
  ...
  if (bass > 0.4) {
    ...
    for (let i = 0; i < rayCount; i++) {
      ...
      const rayGrd = ctx.createLinearGradient(cx, cy, cx + Math.cos(angle) * width, cy + Math.sin(angle) * width);
  ...
  const orbs = [
    { x: 0.3 + Math.sin(t * 0.6) * 0.2, y: 0.4 + Math.cos(t * 0.4) * 0.2, r: 0.6 * (1 + bass * 0.4), h: hTheme[0].h, a: 0.45 },
    { x: 0.7 + Math.cos(t * 0.5) * 0.2, y: 0.6 + Math.sin(t * 0.7) * 0.2, r: 0.5 * (1 + bass * 0.3), h: hTheme[1].h, a: 0.35 },
    { x: 0.5 + Math.sin(t * 0.3) * 0.3, y: 0.3 + Math.cos(t * 0.8) * 0.2, r: 0.55 * (1 + treble * 0.3), h: hTheme[2].h, a: 0.35 },
  ];
  ...
  if (flareAlpha > 0.05) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      ctx.fillStyle = gGrd;
      ctx.beginPath(); ctx.arc(gx, flareY, gSize, 0, Math.PI * 2); ctx.fill();
    }
  }
