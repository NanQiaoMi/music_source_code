...
export const drawOrganicFluid = ({
  ...
  const dx = Math.sin(t * 0.06) * 70,
    dy = Math.cos(t * 0.05) * 50;
  ...
  for (let i = 0; i < ribbonCount; i++) {
    ...
    if (val > 0.7 && bioNodes.length < 4 && i % 3 === 0) {
      ...
      bioNodes.push({
        x: safeWidth * 0.5 + Math.sin(t * 0.8 + i) * safeWidth * 0.25,
        y: ry,
        hue: bHue,
        val,
        alpha,
  ...
  bioNodes.forEach((node, i) => {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(0, 0, s / 2 + 10, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * node.val);
    ctx.stroke();
    ...
    [
      ...
      ctx.beginPath();
      ctx.moveTo((mx * s) / 2, my * (s / 2 - 10));
      ctx.lineTo((mx * s) / 2, (my * s) / 2);
      ctx.lineTo(mx * (s / 2 - 10), (my * s) / 2);
      ctx.stroke();
    });
    ctx.restore();

    // Glass Panel
    const tx = node.x + s / 2 + 28,
      ty = node.y - s / 2;
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(cx - 250, sY);
  ctx.lineTo(cx + 250, sY);
  ctx.stroke();
