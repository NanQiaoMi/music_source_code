...
export const drawGravitationalField = ({
  ctx,
  width,
  height,
  data,
  ...
  for (let i = 0; i < ringCount; i++) {
    ...
    const cosA = Math.cos(sAngle),
      sinA = Math.sin(sAngle);
    const cosR = Math.cos(orbitRot),
      sinR = Math.sin(orbitRot);
  ...
  for (let i = 0; i < gyroCount; i++) {
    ...
    const nx = Math.cos(nAngle) * gRadius,
      ny = Math.sin(nAngle) * gRadius * 0.3;
  ...
  coreGrd.addColorStop(
    0.5,
    `hsla(${(theme.secondary - coreHueShift + 360) % 360}, 100%, 70%, 0.6)`
  );
  ...
  [
    ...
    ctx.scale(sx, sy);
    ctx.beginPath();
    ctx.moveTo(sqSize / 2, sqSize / 2 - bL);
    ctx.lineTo(sqSize / 2, sqSize / 2);
    ctx.lineTo(sqSize / 2 - bL, sqSize / 2);
    ctx.stroke();
    ctx.restore();
  });
  ...
  for (let i = 0; i < 110; i++) {
    const gx = m + i,
      gy = m - 2 + Math.sin(t * 15 + i * 0.2) * (2 + bass * 12);
