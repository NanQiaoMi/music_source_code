...
export const VisualizationProgressBar = memo(
  ({ currentTime, duration, bufferedRanges, onSeek }: VisualizationProgressBarProps) => {
    ...
    const bufferedWidth =
      ...
      duration > 0 &&
      !isNaN(bufferedRanges[bufferedRanges.length - 1].end)
        ? Math.max(
            0,
            Math.min(100, (bufferedRanges[bufferedRanges.length - 1].end / duration) * 100)
          )
        : 0;
